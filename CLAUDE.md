...
* Key data files:
  ...
  * `audit_review.html` → functional audit review page
* Key directories:

  * `docs/` → documentation (ARCHITECTURE.md — pipeline architecture)
...
* Key scripts (4 user-facing, all use stdlib only except translate_catalog.py):
  ...
  * `translate_catalog.py` → translates catalog to Chinese (requires openai package)
